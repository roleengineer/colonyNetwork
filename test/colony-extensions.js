...
contract("Colony Extensions", accounts => {
  let colony;
  let token;
  let colonyNetwork;
  let oneTxExtension;
  ...
  describe("Single transaction payments", () => {
    ...
    it("should not allow an admin to call makePayment on the colony directly", async () => {
      await checkErrorRevert(colony.makePayment(colony.address, accounts[4], 1, token.address, 10), "colony-do-not-call-function-directly");
    });
  });
});
